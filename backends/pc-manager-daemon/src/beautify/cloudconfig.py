...
# -*- coding: utf-8 -*-
### BEGIN LICENSE
# Copyright (C) 2013 National University of Defense Technology(NUDT) & cdos Ltd
...
class CloudConfig(threading.Thread):
    ...
    def get_kuaipan_path(self):
        ...
        kpconf.read(self.home + '/.config/cdos/kuaipan4uk.conf')
    ...
    def real_upload_kysoft_cloud_conf(self):
        ...
        # 快盘没有启动或没有安装，提示用户安装并启动快盘才能上传
            ...
            return
        self.kuaipan_path = self.get_kuaipan_path() + '/[Ubuntu cdos Sync]'
        if not os.path.isdir(self.kuaipan_path):
            os.makedirs(self.kuaipan_path)
        self.kuaipan_path = self.kuaipan_path + '/pc-manager'
        if not os.path.isdir(self.kuaipan_path):
            os.makedirs(self.kuaipan_path)
        self.pc_path = self.home + '/.config/cdos/pc-manager/'
        pc_abs_path = self.pc_path + 'pc-settings-cloud.conf'
        self.save_configuration_to_local_file(pc_abs_path)
        copy(pc_abs_path, self.kuaipan_path)
    ...
    def real_download_kysoft_cloud_conf(self):
        ...
        self.pc_path = self.home + '/.config/cdos/pc-manager/'
        pc_abs_path = self.pc_path + 'pc-settings-cloud.conf'
        if not os.path.exists(pc_abs_path):
            os.mknod(pc_abs_path)
        cloud_abs_path = self.get_kuaipan_path() + '/[Ubuntu cdos Sync]/pc-manager/pc-settings-cloud.conf'
        if not os.path.isfile(cloud_abs_path):
        # 云端没有备份的配置文件
            self.sysdaemon.download_cloud_conf_signal('download_notconf')
            return
        copy(cloud_abs_path, self.pc_path)
        self.use_cloud_configuration(pc_abs_path)
