...
public:
    ...
    //退出登录
    Q_INVOKABLE void logout_cdos_account();
...
//    Q_INVOKABLE QStringList scan_oldkernel_packages_qt();
    ...
    //判断应用是否安装
    Q_INVOKABLE QString checkInstalled(QString pkgName);
    //启动组件
    Q_INVOKABLE void startApplicationsSystem(int type, QString cmdStr);
...
//    void tellAbsPathToCacheModel(QString flag, QString path);
    ...
    //告诉QML界面，标题栏控制按钮位置发生变化了，准备改变自身的控制按钮位置
