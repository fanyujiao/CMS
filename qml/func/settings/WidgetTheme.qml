...
//            }
    ...
    Component.onCompleted: {
        ...
        themeModel.clear();
        for(var i=0; i < syslist.length; i++) {
            if(syslist[i] == "Ambiance" || syslist[i] == "HighContrast" || syslist[i] == "Radiance" || syslist[i] == "cdos-theme"){
            ...
            onClicked: {
                ...
                if(flag) {//系统默认值存在于列表中
                    ...
                    for(q=0; q < mylist.length; q++) {
                        if(mylist[q] == "cdos-theme") {
                            flag = true;
                            break;
                        }
                    }
                    if(flag) {//cdos-theme存在于列表中
                        sessiondispatcher.set_window_theme_qt("cdos-theme");
                        windowcombo.selectedIndex = q;
                    }
                    else {//cdos-theme不存在于列表中
